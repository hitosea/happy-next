...
The Orchestrator lets you define multi-step tasks and dispatch them across multiple AI agents (Claude, Codex, Gemini, Qoder) with dependency management.
...
- **Tasks** — Individual units of work, each with:
  ...
  - A provider (Claude, Codex, Gemini, or Qoder)
  - A model selection
...
Four commands are auto-installed the first time an orchestrating session starts (after upgrading the CLI). Invoke them from a **Claude** or **Codex** session:
...
| Use different agents | "dispatch to Claude/Codex/Gemini/Qoder" |
